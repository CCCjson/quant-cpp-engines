 ...
 *
 * ============================================================
 * 负向对照：把缺陷逐个还原，确认每个用例守的是哪一条
 * ============================================================
 *
 * 「测试绿了」本身不说明任何事 —— 一个永远绿的测试也是绿的。
 * 所以修完之后把两处缺陷**分别**还原，实测各自会让谁变红：
 *
 *   还原的缺陷                           Reused / SameDataTwice   TwoSymbols
 *   ──────────────────────────────────  ──────────────────────  ──────────
 *   on_init() 退回空实现（修复前状态）            🔴 红                ✅ 绿
 *   状态退回全场共用一份（不按标的分槽）           ✅ 绿                🔴 红
 *   两个一起还原                                 🔴 红                🔴 红
 *
 * 两条对角线说明两件事：每个用例都真的有判别力，而且**两个用例互不冗余** ——
 * 删掉任何一个，对应的那一类缺陷就没人守了。
 *
 * 元测试 FixturesActuallyProduceOrders 在四种情形下都绿，符合预期：
 * 它守的是「行情本身有信号」，与这两处缺陷无关。
 */
