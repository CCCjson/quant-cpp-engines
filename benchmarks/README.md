...
**3. 需要可预测的尾部延迟** — 订单簿 p999 = 2,917 ns
...
**3. 开发和修改成本** — 1,431 行 vs 7,866 行

Python 参照引擎 1,431 行做的事，C++ 引擎用了 7,866 行
...
| 指标 | 加索引前 | 加索引后 | 变化 |
|---|---|---|---|
| p50 | 42,209 ns | **84 ns** | **502×** |
| p99 | 99,709 ns | 292 ns | 341× |
| p99.9 | 133,250 ns | 584 ns | 228× |
| 撤单/下单 p50 | 50.6× | 0.13× | —— |
...
注意 `get_depth` 那三行：改之前随簿深从 209 涨到
501 ns，改之后**基本不随簿深变化**了。
