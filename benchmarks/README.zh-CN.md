...
**3. 开发和修改成本** — 1,431 行 vs 9,100 行

Python 参照引擎 1,431 行做的事，C++ 引擎用了 9,100 行
...
| **P7** | 引入 update(bar)/reset() 接口之后，MA_CROSS / MACD / RSI / KDJ @25,000 根全部落在改动前的 ±2% 以内 | ⏳ 待回填<br>⚠️ 这条预测本身写错了：它把 ±2% 的容差钉在**另一天测的基线**上。实测同一份未改动的代码跨会话就漂 2.9%–6.6%（见 results/machine_drift.json），噪声比要测的效应还大。所以改为与**同一次会话**的基线对比，原预测原样留着不改。 |
| **P8** | 接口重构之后，指标金标准 fixture 逐位不变（全量重算路径与增量路径都命中） | ⏳ 待回填 |
| **P9** | 『同一进程连跑两场回测，第二场必须等于单独跑一场』这个测试，在补 on_init() 之前会**红** | ⏳ 待回填 |
| **P10** | 『两个标的装进同一个引擎、共用一个策略实例』这个测试，在补 on_init() 之前也会**红**（跨标的串味） | ⏳ 待回填 |
| **P11** | 删除 ema() / ema_at() 不会让任何测试变红 | ⏳ 待回填 |
