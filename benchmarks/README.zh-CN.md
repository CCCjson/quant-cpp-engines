...
| # | 预测 | 实测 |
|---|---|---|
| **P1** | 指标已经不再是热点：25,000 根 bar 上，四个策略各自的指标计算占该策略整场回测时间的比例，全部 < 15% | ⏳ 待回填 |
| **P2** | KDJ 的 HHV/LLV 窗口扫描占 KDJ 整场回测 < 10%（**这是一条决策规则，不只是预测**） | ⏳ 待回填 |
| **P3** | 若 P2 成立而仍然实现了单调队列，KDJ @25,000 根只会从 5.816 ms 降到 5.65–5.80 ms（省 0–3%），且很可能落在测量噪声里 | ⏳ 待回填 |
| **P4** | BOLLINGER 的 log–log 拟合斜率落在 1.00–1.10（**这个策略从未进过 benchmark，现在不知道答案**） | ⏳ 待回填 |
| **P5** | MOMENTUM 的 log–log 拟合斜率落在 1.00–1.05（**同样从未测过**） | ⏳ 待回填 |
| **P6** | BOLLINGER 与 MOMENTUM @25,000 根的耗时落在 5.0–7.0 ms，与现有四个策略同一量级 | ⏳ 待回填 |
| **P7** | 引入 update(bar)/reset() 接口之后，MA_CROSS / MACD / RSI / KDJ @25,000 根全部落在改动前的 ±2% 以内 | ⏳ 待回填 |
