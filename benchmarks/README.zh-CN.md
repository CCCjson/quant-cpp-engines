...
**3. 开发和修改成本** — 1,431 行 vs 9,479 行

Python 参照引擎 1,431 行做的事，C++ 引擎用了 9,479 行
...
1. ~~**`strategy_context.h` 的 `macd()` / `rsi()` / `kdj()` 改成增量递推。**~~
   ✅ **已完成。** 25,000 根上实测 **240×**，复杂度 O(N²) → O(N)，
...
|---|---|---|---|---|
| 250 | 0.381 | 0.099 | **4×** | ✅ 逐位相同 |
| 1,000 | 2.308 | 0.202 | **11×** | ✅ 逐位相同 |
| 2,500 | 14.604 | 0.503 | **29×** | ✅ 逐位相同 |
| 10,000 | 240.105 | 2.290 | **105×** | ✅ 逐位相同 |
| 25,000 | 1,468.161 | 6.127 | **240×** | ✅ 逐位相同 |
...
| **P9** | 『同一进程连跑两场回测，第二场必须等于单独跑一场』这个测试，在补 on_init() 之前会**红** | ✅ 命中 红过，而且用负向对照复核过。修复前提交（e672f0e）里这个用例就是红的；修完之后把 on_init() 退回空实现（即修复前那种「没人覆写」的状态），ReusedInstanceBehavesLikeAFreshOne 与 RunningTheSameDataTwiceMatchesASingleRun 两个用例立刻变红，而跨标的那个仍绿 —— 说明它们守的确实是跨场这一条。⚠️ 五个策略里 KDJ 是**结构免疫**的：k 从 50 起步，到首个评估 bar 只走两步，必然落在 [22.2, 77.8]，够不着金叉要的 k<20 或死叉要的 k>80。与数据无关。 |
| **P10** | 『两个标的装进同一个引擎、共用一个策略实例』这个测试，在补 on_init() 之前也会**红**（跨标的串味） | ✅ 命中 红过，五个策略全红。负向对照：把状态从按 ctx.symbol 分槽退回全场共用一份，OneInstanceServingTwoSymbolsDoesNotContaminate 立刻变红，而跨场那两个仍绿。⚠️ 修的过程中发现**一个 on_init() 不够**：它每场只调一次，而一场之内多个标的是交替进来的，所以跨标的必须靠分槽解决。这一步是被测试逼出来的，不是一开始就想到的。另外 engine.cpp 那条「现有 9 个策略都是从 ctx.history 现算的，故安全」的注释与代码事实不符，一并改正。 |
